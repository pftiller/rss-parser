...
async function dissectRSS(url) {
  return new Promise((resolve, reject) => {
    ...
    parser.parseURL(url.feed, (err, feed) => {
      if (err) {
        console.error(
          `Failed to parse RSS feed for ${url.program} (${url.feed}):`,
          err.message
        );
...
export async function processAndMergeData() {
  try {
    ...
    //   throw error;
    await insertRowsAsStream(filteredRssData);
    return `Process completed successfully. Processed ${filteredRssData.length} RSS records.`;
  } catch (error) {
    console.error("Error processing data: ", error);
    throw error;
  }
